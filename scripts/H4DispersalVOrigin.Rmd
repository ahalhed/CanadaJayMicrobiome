...
mixedMeta <- left_join(mixedLong, meta_sub) %>%
  ...
  select(JayID, SampleID, JayID2, SampleID2, DistanceFromOrigin, CollectionYear, Territory, Territory2)
# distance that sample 1 is from the origin of sample 2
mixedDF <- mixedMeta[mixedMeta$DistanceFromOrigin<1000,]
```
...
# 2 plots --> one with distances less than 1 KM and all samples
plotDFnear <- left_join(AitchisonLong, mixedDF, by = c("SampleID", "SampleID2")) %>% .[!is.na(.$JayID),]
...
ggplot(plotDFnear, aes(x = DistanceFromOrigin, y = AitchisonDistance)) +
  geom_point() +
