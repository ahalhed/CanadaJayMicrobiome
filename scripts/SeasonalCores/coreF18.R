...
map <- read_tsv("input/jay-met.tsv")[-1,] %>%
  .[which(.$CollectionSeason == "Fall" & .$CollectionYear == 2018),]
...
write.table(occ_abunT, file = "CanadaJayMicrobiome/data/coreJayF18.csv", sep = ",", quote = F, row.names = F)
write.table(core, file = "CanadaJayMicrobiome/data/coreFeaturesF18.tsv", sep = "\t", quote = F, row.names = F)
