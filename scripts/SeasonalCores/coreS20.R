...
map <- read_tsv("input/jay-met.tsv")[-1,] %>%
  .[which(.$CollectionSeason == "Spring" & .$CollectionYear == 2020),]
...
write.table(occ_abunT, file = "CanadaJayMicrobiome/data/coreJayS20.csv", sep = ",", quote = F, row.names = F)
write.table(core, file = "CanadaJayMicrobiome/data/coreFeaturesS20.tsv", sep = "\t", quote = F, row.names = F)
